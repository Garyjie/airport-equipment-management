...
```
使用中 → 备机区/损坏区/送修区 → 库房（stationId=null）
损坏区 → 送修区 → 库房（stationId=null）
送修区 → 备机区 → 库房（stationId=null）
备机区 → 使用中 → 需要手动分配到柜台
```
...
| 版本 | 日期 | 变更内容 |
|------|------|----------|
| v2.2.0 | 2026-06 | 修复局域网访问、修复TypeScript类型推断 |
| v2.1.0 | 2026-06 | 修复状态变更逻辑、添加Electron支持、修复普通用户登录 |
